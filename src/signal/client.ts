...
/**
 * Stream incoming Signal messages via the native signal-cli JSON-RPC
 * WebSocket at /api/v1/rpc.  Incoming messages arrive as JSON-RPC
 * notifications: { method: "receive", params: { envelope: {...} } }.
 * We normalise them into the same SignalSseEvent shape so the existing
 * event handler works unchanged.
 */
export async function streamSignalJsonRpc(params: {
	baseUrl: string;
	abortSignal?: AbortSignal;
	onEvent: (event: SignalSseEvent) => void;
}): Promise<void> {
	const baseUrl = normalizeBaseUrl(params.baseUrl);
	const wsUrl = `${baseUrl.replace(/^http/, "ws")}/api/v1/rpc`;
